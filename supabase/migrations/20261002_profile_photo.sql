alter table public.family_people add column if not exists avatar_image text check(avatar_image is null or (length(avatar_image)<=200000 and avatar_image ~ '^data:image/jpeg;base64,[A-Za-z0-9+/=]+$'));
...
begin
 ...
 if action='save_person' and payload ? 'avatar_image' then
  update public.family_people set avatar_image=nullif(payload->>'avatar_image','') where id=(saved->>'id')::uuid;
 end if;
