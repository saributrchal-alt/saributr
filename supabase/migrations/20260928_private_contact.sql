...
begin
 ...
 return jsonb_set(result, '{people}', coalesce((
  select jsonb_agg((case when exists(select 1 from public.family_accounts own where own.user_id=auth.uid() and own.person_id=(person.value->>'id')::uuid) then person.value else person.value - 'birth_date' - 'phone' end) || jsonb_build_object('avatar_url', (
...
declare target uuid; original jsonb;
begin
