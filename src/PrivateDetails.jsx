...
export default function PrivateDetails({person,own,admin}){
 ...
 useEffect(()=>{if(!own)return;let active=true;supabase.rpc('family_private_details',{person_id:person.id}).then(({data,error})=>{if(active&&!error)setValue(data)});return()=>{active=false}},[own,person.id])
 const data=own?{...person,...value}:value
 return <section><h3>ข้อมูลส่วนตัว</h3>{!own&&admin&&<button type="button" className="btn secondary" style={{touchAction:'none',userSelect:'none'}} onContextMenu={e=>e.preventDefault()} onPointerDown={e=>{if(e.button!==0)return;e.currentTarget.setPointerCapture?.(e.pointerId);reveal()}} onPointerUp={hide} onPointerCancel={hide} onPointerLeave={hide} onLostPointerCapture={hide} onBlur={hide} onKeyDown={e=>{if([' ','Enter'].includes(e.key)){e.preventDefault();reveal()}}} onKeyUp={e=>{if([' ','Enter'].includes(e.key)){e.preventDefault();hide()}}}>ขอดู (กดค้าง)</button>}{data?<dl className="facts"><div><dt>เลขประจำตัวประชาชน</dt><dd>{data.citizen_id||'ยังไม่ระบุ'}</dd></div><div><dt>วันเกิด (ค.ศ.)</dt><dd>{data.birth_date||'ยังไม่ระบุ'}</dd></div><div><dt>เบอร์ติดต่อ</dt><dd>{data.phone||'ยังไม่ระบุ'}</dd></div></dl>:<p>วันเกิดและเบอร์โทรถูกซ่อน</p>}{error&&<p role="alert">{error}</p>}</section>
}
