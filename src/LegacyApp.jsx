...
import { nameDefaults, composeName, matchesName, displayName } from './personNames'
...
const fields = [['first_name','ชื่อปัจจุบัน','text'],['last_name','นามสกุลปัจจุบัน','text'],['birth_first_name','ชื่อเดิมแต่เกิด','text'],['birth_last_name','นามสกุลเดิมแต่เกิด','text'],['nickname','ชื่อเล่น','text'],['birth_date','วันเกิด (ค.ศ.)','date'],['death_date','วันที่เสียชีวิต (ถ้ามี / ค.ศ.)','date'],['birthplace','บ้านเกิด','text']]
function errorText(e) {
  if(['kinship_gender','first_name','last_name','name_title'].some(k=>e.message?.includes(k))) return 'กรุณารัน SQL เพิ่มช่องชื่อและนามสกุลใน Supabase ก่อน แล้วบันทึกอีกครั้ง'
...
function Workspace({user}){
  ...
  const person=people.find(p=>p.id===selected),name=id=>displayName(people.find(p=>p.id===id))
  const connected=relations.filter(r=>r.person_id===selected||r.related_person_id===selected)
  const kinship=deriveKinship(selected,people,relations)
  const groups=[
    ...
    ['พี่และน้อง',kinship.siblings],
    ['ลูกพี่ลูกน้อง',kinship.cousins],
  ...
  function savePerson(e){
    ...
    if(!values.first_name){setError('กรุณาระบุชื่อปัจจุบัน');return}
    const gender=f.get('kinship_gender')||null
    if(!['male','female'].includes(gender)){setError('กรุณาเลือกเพศ');return}
    values.kinship_gender=gender
    values.name_title=f.get('name_title')||''
    values.full_name=composeName(values.name_title, values.first_name, values.last_name)
    if(values.full_name.length>200){setError('ชื่อพร้อมคำนำหน้าต้องไม่เกิน 200 ตัวอักษร');return}
  ...
  return <div className="shell"><header className="topbar"><a className="brand" href="#top"><b>ส</b><span><strong>สายใยครอบครัว</strong><small>ตระกูลสาริบุตร</small></span></a><span className="account-email">{user.email}</span><button className="btn secondary" disabled={busy} onClick={async()=>{const {error}=await supabase.auth.signOut();if(error)setError(errorText(error))}}>ออกจากระบบ</button></header>
    ...
    <div className="toolbar"><input aria-label="ค้นหาสมาชิก" placeholder="ค้นหาชื่อปัจจุบัน ชื่อเดิม นามสกุล หรือชื่อเล่น" value={query} onChange={e=>setQuery(e.target.value)}/><button className="btn secondary" disabled={loading||busy} onClick={()=>{setError('');load().catch(e=>setError(errorText(e)))}}>โหลดใหม่</button></div>
    {loading?<p role="status">กำลังโหลดข้อมูล…</p>:<div className="family-grid"><section className="members-panel"><h2>สมาชิก</h2>{!people.length&&<p>เริ่มด้วยการเพิ่มตัวเอง แล้วเพิ่มพ่อ แม่ และญาติ</p>}{people.filter(p=>matchesName(p,query)).map(p=><button key={p.id} className={'member-card '+(selected===p.id?'selected':'')} onClick={()=>setSelected(p.id)}><span className="avatar green">{p.full_name.slice(0,1)}</span><span><strong>{displayName(p)}</strong><small>{p.nickname||p.birthplace||'ดูรายละเอียด'}</small></span></button>)}</section>
    {person&&<section className="person-panel"><div className="heading"><h2>{displayName(person)}</h2><button className="btn secondary" disabled={busy} onClick={()=>{setError('');setModal({type:'person',person})}}>แก้ไข</button></div><dl className="facts">{[['เพศ',person.kinship_gender==='male'?'ชาย':person.kinship_gender==='female'?'หญิง':null],...fields.map(([key,label])=>[label,['birth_first_name','birth_last_name'].includes(key)?(person[key]?.trim()||'เหมือนปัจจุบัน'):person[key]]),['เรื่องราว',person.biography]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v||'ยังไม่ระบุ'}</dd></div>)}</dl>
    <div className="heading"><h2>เครือญาติของบุคคลนี้</h2><button className="btn secondary" disabled={people.length<2||busy} onClick={()=>{setError('');setModal({type:'relation'})}}>+ เชื่อมญาติ</button></div><p>พี่น้องคำนวณจากพ่อแม่ที่เชื่อมไว้ ส่วนลูกพี่ลูกน้องคำนวณเมื่อพ่อหรือแม่ของทั้งสองคนเป็นพี่น้องกัน ลำดับอายุใช้วันเกิดหรือข้อมูลที่ระบุเอง</p><div className="relation-groups">{groups.map(([label,items])=><section key={label}><h3>{label}</h3>{items.length?items.map(([id,role])=><button className="relative" key={id+role} onClick={()=>setSelected(id)}>{name(id)} <small>({role})</small></button>):<p>ยังไม่ได้เชื่อมข้อมูล</p>}</section>)}</div>
    ...
    {modal&&<div className="backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="dialog-title"><header><h2 id="dialog-title">{modal.type==='person'?(modal.person?'แก้ไขข้อมูล':'เพิ่มบุคคล'):'เชื่อมความสัมพันธ์'}</h2><button aria-label="ปิด" disabled={busy} onClick={()=>setModal(null)}>×</button></header>{error&&<p className="alert" role="alert">{error}</p>}{modal.type==='person'?<form className="form" onSubmit={savePerson}><label>คำนำหน้านาม<select name="name_title" defaultValue={nameDefaults(modal.person).name_title}><option value="">ไม่ระบุ / คำนำหน้าอื่น</option>{titleGroups.map(([group,titles])=><optgroup key={group} label={group}>{titles.map(title=><option key={title} value={title}>{title}</option>)}</optgroup>)}</select></label><p>หากใช้คำนำหน้าอื่นหรือยศแบบย่อ ให้เลือก “ไม่ระบุ / คำนำหน้าอื่น” แล้วกรอกพร้อมชื่อ</p><label>เพศ<select required name="kinship_gender" defaultValue={modal.person?.kinship_gender||''}><option value="" disabled>เลือกเพศ</option><option value="male">ชาย</option><option value="female">หญิง</option></select></label><p>ใช้เพศเป็นเกณฑ์เรียกพี่ชาย พี่สาว น้องชาย และน้องสาว</p>{modal.person&&modal.person.first_name==null&&<p>ข้อมูลเดิมเก็บชื่อรวมไว้ กรุณาตรวจสอบและแยกชื่อกับนามสกุลก่อนบันทึก</p>}{fields.map(([key,label,type])=><label key={key}>{label}<input name={key} type={type} placeholder={key==='birth_first_name'?'กรอกเมื่อชื่อปัจจุบันไม่ตรงชื่อแต่เกิด':key==='birth_last_name'?'กรอกเมื่อนามสกุลปัจจุบันไม่ตรงนามสกุลแต่เกิด':undefined} defaultValue={nameDefaults(modal.person)[key]??modal.person?.[key]??''} required={key==='first_name'} maxLength={type==='text'?200:undefined}/></label>)}<label>ประวัติและเรื่องราว<textarea name="biography" rows={4} defaultValue={modal.person?.biography||''}/></label><button className="btn primary" disabled={busy}>{busy?'กำลังบันทึก…':'บันทึกข้อมูล'}</button></form>:<form className="form" onSubmit={saveRelation}><label>บุคคล<select name="person_id" defaultValue={selected}>{people.map(p=><option key={p.id} value={p.id}>{displayName(p)}</option>)}</select></label><label>ความสัมพันธ์<select name="relationship_type">{Object.entries(labels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label><label>บุคคลอีกคน<select name="related_person_id" defaultValue={people.find(p=>p.id!==selected)?.id}>{people.map(p=><option key={p.id} value={p.id}>{displayName(p)}</option>)}</select></label><p>ตัวอย่าง: เลือกชื่อพี่ → เป็นพี่ของ → เลือกชื่อน้อง</p><button className="btn primary" disabled={busy}>{busy?'กำลังบันทึก…':'บันทึกความสัมพันธ์'}</button></form>}</section></div>}
  </div>
}
