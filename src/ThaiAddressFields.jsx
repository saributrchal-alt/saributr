...
export default function ThaiAddressFields({house,prefix=''}){
 ...
 <input type="hidden" name={prefix+'province'} value={p?.name||''}/><input type="hidden" name={prefix+'district'} value={d?.name||''}/><input type="hidden" name={prefix+'subdistrict'} value={s?.name||''}/>
 <label>รหัสไปรษณีย์<input name={prefix+'postal_code'} required inputMode="numeric" pattern="[0-9]{5}" maxLength={5} value={zip} onChange={e=>setZip(e.target.value)}/></label><small>เติมรหัสจากพื้นที่ที่เลือก ตรวจสอบให้ตรงกับที่อยู่จัดส่งจริง</small>
 </>
}
