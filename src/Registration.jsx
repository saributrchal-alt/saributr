...
export default function RegistrationGate({user,children}){
 ...
 {user.email&&<><hr/><h2>ตั้งรหัสผ่านสำหรับครั้งถัดไป</h2>{passwordSet?<p className="success">ตั้งรหัสผ่านแล้ว</p>:<PasswordForm onDone={()=>setPasswordSet(true)}/>}</>}
 <p>ผู้สมัครผ่าน LINE สามารถใช้ปุ่ม LINE เข้าสู่ระบบในครั้งถัดไป</p><button className="btn secondary" onClick={async()=>{const {error}=await supabase.auth.signOut();if(error)setError(error.message)}}>ออกจากระบบ</button>
 </section></main>
}
export function RegistrationAdmin({onChanged}){
 const [rows,setRows]=useState([]),[error,setError]=useState(''),[busy,setBusy]=useState(false),[notes,setNotes]=useState({}),[loading,setLoading]=useState(true)
 async function load(){setLoading(true);setError('');try{const {data,error}=await supabase.rpc('family_registration_admin',{action:'list'});if(error)throw error;setRows(data||[])}catch(e){setError(e.message);setRows([])}finally{setLoading(false)}}
 useEffect(()=>{load()},[])
