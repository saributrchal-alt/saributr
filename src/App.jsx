...
import { nameDefaults, composeName, matchesName } from './personNames'
...
function Workspace({user}){
  ...
  return <div className="shell"><header className="topbar"><a className="brand" href="#top"><b>ส</b><span><strong>สายใยครอบครัว</strong><small>ตระกูลสาริบุตร</small></span></a><span className="account-email">{user.email}</span><button className="btn secondary" disabled={busy} onClick={async()=>{const {error}=await supabase.auth.signOut();if(error)setError(errorText(error))}}>ออกจากระบบ</button></header>
    ...
    {person&&<section className="person-panel"><div className="heading"><h2>{person.full_name}</h2><button className="btn secondary" disabled={busy} onClick={()=>{setError('');setModal({type:'person',person})}}>แก้ไข</button></div><dl className="facts">{[['เพศ',person.kinship_gender==='male'?'ชาย':person.kinship_gender==='female'?'หญิง':null],...fields.map(([key,label])=>[label,person[key]]),['เรื่องราว',person.biography]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v||'ยังไม่ระบุ'}</dd></div>)}</dl>
