...
import { titleGroups, splitName, joinName } from './nameTitle'
...
function errorText(e) {
  if(e.message?.includes('kinship_gender')) return 'กรุณารัน SQL เพิ่มช่องคำเรียกชาย/หญิงใน Supabase ก่อน แล้วบันทึกอีกครั้ง'
...
function Workspace({user}){
  ...
  function savePerson(e){
    ...
    if(gender||modal.person?.kinship_gender)values.kinship_gender=gender
    values.full_name=joinName(f.get('name_title')||'', values.full_name)
  ...
  return <div className="shell"><header className="topbar"><a className="brand" href="#top"><b>ส</b><span><strong>สายใยครอบครัว</strong><small>ตระกูลสาริบุตร</small></span></a><span className="account-email">{user.email}</span><button className="btn secondary" disabled={busy} onClick={async()=>{const {error}=await supabase.auth.signOut();if(error)setError(errorText(error))}}>ออกจากระบบ</button></header>
    ...
    {modal&&<div className="backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="dialog-title"><header><h2 id="dialog-title">{modal.type==='person'?(modal.person?'แก้ไขข้อมูล':'เพิ่มบุคคล'):'เชื่อมความสัมพันธ์'}</h2><button aria-label="ปิด" disabled={busy} onClick={()=>setModal(null)}>×</button></header>{error&&<p className="alert" role="alert">{error}</p>}{modal.type==='person'?<form className="form" onSubmit={savePerson}><label>คำนำหน้านาม<select name="name_title" defaultValue={splitName(modal.person?.full_name).title}><option value="">ไม่ระบุ / คำนำหน้าอื่น</option>{titleGroups.map(([group,titles])=><optgroup key={group} label={group}>{titles.map(title=><option key={title} value={title}>{title}</option>)}</optgroup>)}</select></label><p>หากใช้คำนำหน้าอื่นหรือยศแบบย่อ ให้เลือก “ไม่ระบุ / คำนำหน้าอื่น” แล้วกรอกพร้อมชื่อ</p><label>คำเรียกในเครือญาติ<select name="kinship_gender" defaultValue={modal.person?.kinship_gender||''}><option value="">ตามคำนำหน้า / ยังไม่ระบุ</option><option value="male">ชาย — พี่ชาย / น้องชาย</option><option value="female">หญิง — พี่สาว / น้องสาว</option></select></label><p>ยศทหารและตำรวจไม่ระบุเพศ กรุณาเลือกคำเรียกหากต้องการแยกชาย–หญิง</p>{fields.map(([key,label,type])=><label key={key}>{label}<input name={key} type={type} defaultValue={key==='full_name'?splitName(modal.person?.full_name).name:(modal.person?.[key]||'')} required={key==='full_name'} maxLength={key==='full_name'?200:undefined}/></label>)}<label>ประวัติและเรื่องราว<textarea name="biography" rows={4} defaultValue={modal.person?.biography||''}/></label><button className="btn primary" disabled={busy}>{busy?'กำลังบันทึก…':'บันทึกข้อมูล'}</button></form>:<form className="form" onSubmit={saveRelation}><label>บุคคล<select name="person_id" defaultValue={selected}>{people.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></label><label>ความสัมพันธ์<select name="relationship_type">{Object.entries(labels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label><label>บุคคลอีกคน<select name="related_person_id" defaultValue={people.find(p=>p.id!==selected)?.id}>{people.map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></label><p>ตัวอย่าง: เลือกชื่อพี่ → เป็นพี่ของ → เลือกชื่อน้อง</p><button className="btn primary" disabled={busy}>{busy?'กำลังบันทึก…':'บันทึกความสัมพันธ์'}</button></form>}</section></div>}
  </div>
}
